...
test.describe('W3C Web Annotations', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/render/w3c.html');
  });
  ...
  test('click on W3C annotation fires event', async ({ page }) => {
    ...
    const annotation = page
      .locator(
        '#w3c-interactive [data-annotation-uid][data-annotation-role="fill"]',
      )
      .first();
  ...
  test('create W3C annotation via drag outputs W3C format', async ({
    page,
  }) => {
    const container = page.locator('#w3c-interactive');
    const eventLog = page.locator('#event-log');
  ...
  test('W3C annotation preserves TextPositionSelector format', async ({
    page,
  }) => {
    ...
    expect(firstAnnotation['@context']).toBe(
  ...
  test('W3C adapter correctly parses annotation positions', async ({
    page,
  }) => {
    const container = page.locator('#w3c-basic');
    await expect(container).toBeVisible();
  ...
  test('hover on W3C annotation shows highlight', async ({ page }) => {
    ...
    const annotation = page
      .locator('#w3c-basic [data-annotation-uid][data-annotation-role="fill"]')
      .first();
