  ...
  tableTestDescribe<'id' | 'canEdit' | 'canCreate'>`
    ...
    test(`Create: ${canCreate}`, async ({ page }) => {
      ...
      await getLog(page, id).expect(
        canCreate ? 'annotation-create--start' : '',
      );

      await mouseMove.onMouseDrag({ x: 80, y });
      await getLog(page, id).expect(canCreate ? 'annotation-create--move' : '');

      await mouseMove.onMouseEnd();
      await getLog(page, id).expect(canCreate ? 'annotation-create--end' : '');
    ...
    test(`Edit: ${canEdit}`, async ({ page }) => {
      ...
      await getLog(page, id).expect(canEdit ? 'annotation-edit--start' : '');

      await mouseMove.onMouseDrag({ x: x - 20, y });
      await getLog(page, id).expect(canEdit ? 'annotation-edit--move' : '');

      await mouseMove.onMouseEnd();
      await getLog(page, id).expect(canEdit ? 'annotation-edit--end' : '');
